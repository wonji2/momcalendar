...
// 🔴 2026-08-16 전면 개편 — 사장님 지적 "핫딜 대분류·소분류 오탐이 많아. 린넨 슬릿 스커트가 왜 리빙 생활용품이야"
//   원인 ① 패션(옷·신발·모자) 규칙이 아예 없어 전부 '리빙/생활용품' 기본값으로 떨어졌다.
//        ② 매칭 순서가 잘못돼 '산양유 비누 화이트우유' 가 식품(우유)으로, '바디워시 복숭아' 가 식품(복숭아)으로 샜다.
//        ③ 증정품 문구까지 읽어 '켈로그 + 증정(에센스)' 가 뷰티로 갔다.
//   ⚠ 순서가 곧 우선순위다. 세정·위생(비누·바디워시)을 **식품보다 먼저** 둘 것.
//   실측 검증: 기존 애드픽 59건 전수 대조 → 56건 정분류 / 3건 드롭 / 오분류 0
const AP_CAT: [RegExp, string, string][] = [
  [/기저귀|분유|이유식|젖병|유모차|카시트|아기띠|치발기|유아용|아동용|어린이집|장난감|블럭|교구/, "육아", "육아용품"],
  [/강아지|고양이|반려|사료/, "반려동물", "반려용품"],
  [/청소기|밥솥|에어프라이어|냉장고|세탁기|건조기|이어폰|노트북|모니터|선풍기|가습기|정수기|드라이기|고데기|커피머신|블렌더|믹서기|제습기|공기청정/, "가전", "생활가전"],
  [/스커트|원피스|블라우스|가디건|니트|맨투맨|후드티|티셔츠|반팔|긴팔|청바지|슬랙스|반바지|레깅스|자켓|재킷|코트|점퍼|패딩|정장|수영복|래시가드|캐미솔(?!.*이불)/, "패션", "의류"],
  [/운동화|스니커즈|샌들|슬리퍼|구두|부츠|워커|플랫슈즈|크록스/, "패션", "신발"],
  [/버킷햇|벙거지|캡모자|볼캡|비니|양말|스타킹|벨트|스카프|머플러|등산모자/, "패션", "잡화"],
  [/백팩|크로스백|토트백|숄더백|에코백|캐리어|여행가방/, "패션", "가방"],
  [/유산균|오메가|비타민|홍삼|루테인|콜라겐|프로바이오|밀크씨슬|영양제|마데카솔|연고|파스|밴드에이드|구강청결/, "건강", "건강식품"],
  // 세정·위생 — 식품보다 먼저 (비누·바디워시가 '우유·복숭아'로 새는 것 방지)
  [/휴지|화장지|키친타월|물티슈|비데|세제|섬유유연제|샴푸|린스|바디워시|비누|치약|칫솔|세정|위생|생리대|라이너|락스|수세미|지퍼백|비닐장갑|변기|발매트|욕실매트|화장실매트|스프레이건|샤워기|수전/, "생필품", "생활용품"],
  // ⚠ '스킨'만 넣으면 "심리스브라-스킨(L)" 같은 색상표기에 걸린다 → 스킨케어/스킨토너로 좁힘
  [/스킨케어|스킨토너|로션|크림(?!.*(치약|과자))|앰플|에센스|선크림|썬크림|쿠션|틴트|립스틱|향수|클렌징|클렌저|토너|마스크팩|세럼|아이크림|파운데이션|팩트/, "뷰티", "화장품"],
  [/라면|과자|간식|음료|커피|생수|한우|한돈|삼겹|정육|햅쌀|즉석|간편식|반찬|견과|과일|우유|만두|김치|닭가슴살|곰탕|콜라|사이다|주스|초코|시리얼|올리브오일|참기름|비비고|프링글스|카프리썬|복숭아|고구마|계란|달걀|쌀\b/, "식품", "가공식품"],
  [/이불|침구|베개|매트리스|커튼|블라인드|러그|카펫|수납|정리함|행거|선반|쇼파|소파|의자|책상|식탁|테이블|콘솔|화장대|타월|수건|냄비|프라이팬|쿡웨어|커트러리|그릇|트레이|텀블러|보온병|멀티탭|쿨매트|패드/, "리빙", "홈리빙"],
];
...
// 증정·사은품 뒷부분은 분류에 쓰지 않는다 ('켈로그 + 증정(에센스)' 가 뷰티로 가던 오탐)
function apCore(name: string): string {
  return String(name).split(/\+\s*증정|증정\(|사은품|\[증정/)[0];
}
function apCat(name: string): [string, string] | null {
  const n = apCore(name);
  if (AP_BLOCK.test(n) || AP_LUXURY.test(n)) return null;
  for (const [re, a, b] of AP_CAT) if (re.test(n)) return [a, b];
  // 🔴 예전엔 못 맞히면 '리빙/생활용품' 으로 넣었다 → 스커트·모자·음료가 전부 리빙으로 쌓였다.
  //   엉뚱한 분류는 손님을 헷갈리게 하니 차라리 안 올린다 (사장님 지적 2026-08-16).
  return null;
}
...
Deno.serve(async (req) => {
  ...
  // 🔴 2026-08-19 애드픽 자동등록 중단 (사장님 지시 "애드픽은 근거없으면 걍 빼버려야할듯")
  //   애드픽의 price_org(정가)는 애드픽이 스스로 부르는 값이라 우리가 검증할 수단이 없다.
  //   토스는 detail 로 현재가를 되묻고, 쿠팡은 가격이력으로 근거를 만드는데 애드픽만 주장을 옮겨 적었다.
  //   실측 지적: 차렵이불 49,900(44%) · CJ유산균 39,000(40%) — 할인율은 통과했지만 금액이 핫딜답지 않았다.
  //   ⚠ 수집·만료점검(goldcheck)은 그대로 둔다. 이미 올라간 카드 관리가 필요하고, 되살릴 땐 이 값만 true 로.
  const AP_ENABLED = false;   // ← 애드픽 자동등록 스위치
  try {
    if (!AP_ENABLED) throw new Error("애드픽 자동등록 중단(근거 검증 불가)");
