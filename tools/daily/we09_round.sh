#!/usr/bin/env bash
# 공구연구소(we09.lab) 데일리 → 승인표 → 게이트 → 등록 (예약작업 momcal-we09, 매일 08:10·13:10 (2026-09-16 신설, jupjup_round.sh 복제))
...
LOG=scratchpad/we09_log.txt
DAY=$(date +%F); TS=$(date +%H%M)
log(){ echo "[$DAY $(date +%H:%M)] $*" | tee -a "$LOG"; }
RAW=scratchpad/_we09_${DAY}_${TS}.md
"$N" tools/daily/we09_parse.mjs "$RAW" > scratchpad/_we09.out 2>&1 || { log "🔴 파싱 실패: $(tail -1 scratchpad/_we09.out)"; exit 1; }
log "$(grep -E '^✅|^처리할|^조각|^핸들' scratchpad/_we09.out | head -1)"
...
f="scratchpad/승인대기_we09_${DAY}_${TS}.md"
...
# 분류 못 채운 행은 보류
mkdir -p scratchpad/we09
grep -E '^\| [0-9]+ \|[^|]*\|[^|]*\|[^|]*\|[^|]*\| *\|' "$f" >> "scratchpad/we09/보류_$DAY.md" 2>/dev/null
sed -i -E '/^\| [0-9]+ \|[^|]*\|[^|]*\|[^|]*\|[^|]*\| *\|/d' "$f"
grep -q '^| [0-9]' "$f" || { log "분류된 행 0 — 끝 (보류 scratchpad/we09/보류_$DAY.md)"; exit 0; }
...
if [ -n "$SPLIT" ]; then for h in $SPLIT; do grep -F "| $h |" "$f" >> "scratchpad/we09/보류_$DAY.md"; sed -i "/| $h |\$/d" "$f"; done; log "핸들갈림 보류: $SPLIT"; grep -q '^| [0-9]' "$f" || exit 0; OUT=$(gate); fi
if echo "$OUT" | grep -q '"already_in_db": [1-9]'; then
  sed '/^select count/,$d' scratchpad/_chk.sql > scratchpad/_chk_list_we09.sql
  echo "select insta||'/'||open_date||'/'||name as row from j where insta is not null and dup order by 1;" >> scratchpad/_chk_list_we09.sql
  "$SB" db query --linked --output-format json -f scratchpad/_chk_list_we09.sql 2>/dev/null | grep -o '"row": "[^"]*"' | sed 's/"row": "//; s/"$//' > "$RAW.dup"
...
log "✅ 등록 $TOT 행"
mkdir -p scratchpad/등록완료/무인_공구연구소 && cp "$f" "scratchpad/등록완료/무인_공구연구소/${DAY}_${TS}_${TOT}건.md"
