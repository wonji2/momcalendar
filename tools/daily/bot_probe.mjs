...
const CASES = [
  ...
  ['말깎임', '돌잡이', 'card', '', '돌잡이'],
  // 🔴 검색은 상품명만 본다 (사장님 지시 2026-09-05). 셀러명·핸들을 보면 안 찾은 상품이 섞인다.
  //    「포도」 에 셀러 '포도네'(podomami) 의 흉터스틱이, 「와이」 에 '와이블랑' 상품이 나갔다.
  //    ⚠ 대가: 「우유맘 공구 알려줘」 같은 셀러 이름 검색은 이제 안 된다 — 그게 지시다. 되돌리지 말 것.
  ['셀러명제외', '포도 있어?', 'text', '', ''],
  ['셀러명제외', '와이', 'any', '', ''],   // 상품명에 '와이' 가 든 공구(이지업 와이더커버)가 있으면 카드가 맞다 — DB 상태 따라 달라져 any
  ...
  ['말깎임', '쌀이 있어?', 'card', '쌀', ''],   // AI 가 q=쌀 로 푼다 (2026-09-06)   // 조사가 떨어져 '쌀' 로 찾는다(건수 대조)
  // 🔴 셀러명에 걸려 엉뚱한 답이 나가던 것 — '국이' 가 셀러 미국이맘 14건(캐리어·의자)을 물어왔다
  // 🔴 2026-09-06 사장님 지시: 조사 '이' 를 떼는 규칙 폐기. 아래 4건은 AI 해석이 붙기 전까지 '없어요' 가 정답이다.
  ['말깎임', '국이 있어?', 'card', '국', ''],   // AI 해석(q=국) → 동결건조국·국밥 (2026-09-06 AI 연결)
  ['말깎임', '책이 있어?', 'card', '책', ''],   // cKw=1(책이좋아) — 그 말이 든 상품이 있으면 안 자른다
  // 🔴 2026-09-05 검증 6차: 접미 토큰으로 가르려다 **품목어가 깎였다**(종이→'종' 20건 중 19건 무관).
  //    한국어 품목어는 상품명에서 낱말 **앞**에 온다(종이접기·명이나물·송이고버섯).
  //    이 셋은 **자르면 안 되는** 쪽이다. 지우지 말 것.
  ['말깎임', '종이 있어?', 'card', '종이', '종이'],
  // ⚠ 진행중 '명이나물' 이 있을 때만 card 다 — DB 상태에 묶인 기대값이라 any (2026-09-09, 실리만·하베브릭스와 같은 이유)
  ['말깎임', '명이 있어?', 'any'],
  ...
  // 🔴 2026-09-05 검증 4차: 건수 대조 2번째 조건이 **브랜드를 새로 깎았다**.
  //    카이 2↔카 105 · 조이 4↔조 61 · 뉴이 2↔뉴 28 · 블루이 1↔블루 20 —
  //    「카이」가 보라카이 2건을 통째로 빼먹고 헤이홈 카메라·돈카츠를 줬다.
  //    이 넷은 **자르면 안 되는** 쪽이다. 지우지 말 것.
  ['말깎임', '카이', 'card', '카이', '카이'],
  ['말깎임', '조이', 'card', '조이', '조이'],
  ['말깎임', '뉴이', 'card', '뉴이', '뉴이'],
  ['말깎임', '블루이', 'card', '블루이', '블루이'],
  ['말깎임', '컵을 찾아줘', 'card', '컵', ''],
  ...
  ['말깎임', '선풍기가', 'text', '', '선풍기'],
  // 끝 글자가 조사처럼 보이지만 **낱말 자체**인 것 — 자르면 안 된다(NOT_JOSA, 2026-09-05).
  //   오이→'오' 226건 · 먹이→'먹' 4건 이 나가던 것. 둘 다 DB 0건이라 '없어요' 가 정답. 지우지 말 것.
  ['말깎임', '오이 있어?', 'text', '', '오이'],
  ['말깎임', '먹이 있어?', 'text', '', '먹이'],
  // 「도」로 끝나는 말은 CUT_TAILS 가 한 바퀴 안에서 같이 지운다 — stripTail 안쪽 가드가 막는다.
  ['말깎임', '독도 있어?', 'text', '', '독도'],
  ['말깎임', '완도 있어?', 'text', '', '완도'],
  ['말깎임', '거제도 있어?', 'text', '', '거제도'],
  // 반대로 진짜 조사 '도' 는 그대로 떼야 한다(회귀)
  ['말깎임', '만두도 있어?', 'card', '만두', ''],
  ['말깎임', '기저귀도 있어?', 'card', '기저귀', ''],
  // ── 브랜드
...
// 🧠 문장형 발화 — 2026-09-06 밤부터 AI 해석(ANTHROPIC_API_KEY)이 라이브다. **기본 실행에 포함**한다.
//   키가 회수되거나 모델이 죽으면 이 묶음이 text 로 떨어져 경보가 나야 한다(검증 지적: 전건이 사전에 있어 AI 가 죽어도 probe 가 통과했다).
//   🐤 카나리아: 날짜가 붙은 문장은 사전에 없어 **매일 첫 실행 때 반드시 AI 를 탄다**(하루 1행·1.1원). 규칙만으로는 못 푸는 문장이라 AI 가 죽으면 text.
const MMDD = new Date(Date.now() + 9 * 3600e3).toISOString().slice(5, 10).replace('-', '');
CASES.push(
  ['문장', '어제 봤던 물티슈 그거 아직 하나요 ' + MMDD, 'card', '물티슈'],   // 🐤 AI 생존 카나리아
  ['핫딜', '물티슈 특가 있어?', 'card', '물티슈'],   // 공구 먼저 (사장님 2026-09-06)
  ['핫딜', '핫딜 뭐 있어', 'text'],                  // 상품 낱말 없음 → 안내
  // 실손님 재현 (2026-09-07 새벽 — 인사·감사 규칙과 AI 가 브랜드를 먹던 것)
  ['실손님', '땡스소윤', 'any', '', '땡스소윤'],                 // '땡스' 감사 규칙에 먹혀 '저도 좋아요' 가 나갔다 (DB 4건)
  ['실손님', '안녕하세요 하베브릭스 장난감 공구일정 알고싶어요', 'any', '', '하베브릭스'],   // 인사 접두어 → 도움말이 나갔다
  ...
  ['판정', '디자인스킨', 'any', '', '디자인스킨'],           // 플레이테이블 별칭은 유지
  // 검증 지적 2026-09-08 (인사 접두어·감사 규칙이 브랜드를 깎던 것)
  ['검증', '하이드로플라스크 텀블러 있어?', 'any', '', '하이드로플라스크'],   // '하이' 접두어 제거에 낱말 경계가 없어 '드로플라스크' 가 됐다
  ['검증', '헬로디노 바운서', 'any', '', '헬로디노'],
  ['검증', '땡스소윤 있어?', 'card', '땡스소윤'],                    // 감사 가드가 u 로 세서 감사 답이 나갔다 → kw 로
  ['문장', '기저귀 싸게 파는데 없나', 'card', '기저귀'], ['문장', '삼겹살 말고 소고기', 'card', '소고기'],
  ['문장', '어제 봤던 물티슈 그거 아직 해?', 'card', '물티슈'], ['문장', '이번주에 뭐 사면 좋을까', 'card'],
  ['문장', '물티슈요', 'card', '물티슈'],   // 국이·만두도 는 말깎임 묶음에 이미 있다(중복 제거 2026-09-06)
);
