...
for (const p of posts) {
  ...
  if (openedByPost && (/(은|는|으로|에서|에게|이라|라고)\s|\s(합리적인|제품으로|가격으로)/.test(name) || /(으로|에서|에게|하게|처럼)$/.test(name) || name.replace(/\s/g, '').length < 4)) { why(`상품명 문장형(게시일규칙): ${name}`); continue; }   // 2026-09-16 "제주갈치세트는 합리적인 가격으로"·"휴대용으로" 류 — 중간·끝 조사로 잡는다(메모리 sentence-fragment-shows-mid-token)
